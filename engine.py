...
class RollingBrain:
    ...
    def solve_full_day(self, load_f, solar_f, price_f, demand_rate, vpp_f=None):
        """Solve one-day least-cost dispatch with monthly peak carryover."""
        n = 96
        if vpp_f is None:
            vpp_f = np.zeros(n)
        ...
        # Grid load <= monthly peak variable
        ...
        # Never optimize a monthly peak below a peak already observed.
    ...
    def update_state(self, c, d, actual_net):
        self.soc = max(
            0,
            min(self.cap, self.soc + (c * self.eta * 0.25) - (d / self.eta * 0.25)),
        )
...
def fixed_rule_dispatch(load, solar, timestamps, cap, pwr, soc, eta=0.93):
    """
    Simple realistic baseline: charge from grid during off-peak and discharge
    during the defined 16:00-21:00 peak window. This is intentionally
    deterministic and contains no future information.
    """
    n = len(load)
    charge = np.zeros(n)
    ...
    for i, ts in enumerate(timestamps):
        net_load = max(float(load[i] - solar[i]), 0.0)
        is_peak = 16 <= ts.hour < 21
        is_offpeak = ts.hour < 16 or ts.hour >= 21

        if is_peak and current_soc > 0:
            max_energy = min(pwr * 0.25 / eta, current_soc)
            discharge[i] = min(pwr, max_energy * eta / 0.25)
